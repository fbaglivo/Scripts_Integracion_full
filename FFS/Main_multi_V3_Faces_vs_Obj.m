...
l_inf=-200/1000; %s 
w_size=500/1000; % 
offset=1000/1000; % (1000 Retrieval  600 retrieval)  
delta=10/1000;
...
%     tiempo=tiempo-40;
    ...
    if strcmp('Features',tipo)
        condicion=2; 
    elseif strcmp('Binding',tipo)
        condicion=1;
    else
        condicion=0;
    end   
    for i=1:size(Pac_Faces.cond(condicion).data)
        ...
        DATA={squeeze(Pac_Faces.cond(condicion).data(i,init:endi,:)),squeeze(Pac_Objects.cond(condicion).data(i,init:endi,:))};
