...
for k=1:size(conf.test_type,2) %
    ...
    for u=1:size(conf.stim_type,2)
        ...
        diff=Lofreq.diff-Hifreq.diff;
        
        limit=max(max(abs(diff)));         
        M=make_triang_edge(diff,-0.5*limit,0.5*limit);
...
%         set(gca,'fontsize',36)
%         
%         text(1500,-2300,['P12 - wSMI diff' cell2mat(conf.stim_type(u)) '.jpg']);
%         
%         %%+l_inf
%         
%         h=gcf;
...
%         print(gcf, '-dpdf', ['GraphBrain\Pdf\P12-Diff'  cell2mat(conf.stim_type(u)) '.pdf'])
%         
