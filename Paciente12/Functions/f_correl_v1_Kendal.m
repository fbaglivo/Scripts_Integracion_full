function f_correl_v1_kendal(tipo)
...
for l=1:size(offset_v,2)
    ...
    for trial=1:size(data.condp(1).window,3);
        ...
        [rb(:,:,trial) pb(:,:,trial)]=corr(m1,m2,'type','Kendall');
    ...
    for trial=1:size(data.condp(2).window,3);
        ...
        [rf(:,:,trial) pf(:,:,trial)]=corr(m1f,m2f,'type','Kendal');
...
save([out_path_var tipo '_corr_Kendal_noref' ],'rf','rb','bind','feat','Ps','T')
