function [erpsMapsByTrial, meanERPSMaps, R, Pboot, Rboot, ERP, freqs, timesout, mbase, maskersp, maskitc, g,Pboottrialsmean] = PlotERPSMap2(EEG,channels,tlimits,cycles,frequencyRange,alpha,fdrCorrect,titleName,weightedSignificance,surroundingsWeight,scale,baseline,basenorm,erpsMax,marktimes)
...
for num = channels
    counter = counter + 1;
    [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mypop_newtimef2(EEG, typeproc, num, tlimits, cycles, frequencyRange,alpha,fdrCorrect,scale,baseline,basenorm,erpsMax,marktimes, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'mcorrect','fdr');
