...
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mypop_newtimef2(EEG, typeproc, num, tlimits, cycles,freqRange,alpha,fdrCorrect,scale,baseline,basenorm,erpsmax,marktimes, varargin );
varargout{1} = '';
lastcom = num;
...
if alpha ~= 0
    [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 4, 'basenorm', basenormvalue,'alpha', alpha,'mcorrect','fdr','erspmax',erpsmax,'marktimes',marktimes);
else
    [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 4, 'basenorm', basenormvalue,'mcorrect','fdr','erspmax',erpsmax,'marktimes',marktimes);
end
