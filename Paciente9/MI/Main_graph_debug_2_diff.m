...
for k=1:size(conf.test_type,2) %
    ...
    for u=1:size(conf.stim_type,2)
        ...
        file= ['P9_' cell2mat(conf.stim_type(u))   '_' num2str(tau(1)) '1000'];
        Hifreq=load([file '.mat']);
        
        
        file= ['P9_' cell2mat(conf.stim_type(u))   '_' num2str(tau(2)) '1000'];
        Lofreq=load([file '.mat']);
        
        Lofreq.WSMI_cond1(find(Lofreq.Ps>0.001))=0;
        Lofreq.WSMI_cond2(find(Lofreq.Ps>0.001))=0;
        Lofreq.diff=-Lofreq.WSMI_cond1+Lofreq.WSMI_cond2; %Bind (neg)-BLUE / Feat (pos) RED
  
        Hifreq.WSMI_cond1(find(Hifreq.Ps>0.001))=0;
        Hifreq.WSMI_cond2(find(Hifreq.Ps>0.001))=0;
