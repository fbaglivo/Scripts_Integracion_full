...
% tipo='Decision_vs'
tipo='PostRetention';  %%canal desde 41
test='Faces';
...
%                 gcondp(u).data=squeeze(cat(2,gcondp(u).data,squeeze(cond(u).data(roi(h).chan(p),:,:))));          
        ...
        %
%         for pp=1:size(gcond(u).data,2)
%            
%             gcond(u).data(:,pp)=(gcond(u).data(:,pp)-min(gcond(u).data(:,pp)))/(max(gcond(u).data(:,pp))-min(gcond(u).data(:,pp)));
%             
%         end
%         
...
%         Gcondp(u).mean(:,h)=mean(gcondp(u).data,2);
        Gcond(u).mean(:,h)=(Gcond(u).mean(:,h)-min(Gcond(u).mean(:,h)))/(max(Gcond(u).mean(:,h))-min(Gcond(u).mean(:,h)));
       
        
        Gcond(u).mean(:,h)=Gcond(u).mean(:,h)-mean(Gcond(u).mean(1:20,h),1);
        
        Gcond(u).std(:,h)=std(gcond(u).data,0,2);
        Gcond(u).std(:,h)=(Gcond(u).std(:,h)-min(Gcond(u).std(:,h)))/(max(Gcond(u).std(:,h))-min(Gcond(u).std(:,h)));
